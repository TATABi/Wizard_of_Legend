...
: CGameState(g), map(740, 918, &g_character)			//角色在地圖上的位置			
{
...
void CGameStateRun_Town::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (delay_counter < 0)
	{
		...
		case FLAG_TOWN_NORMAL:			//一般狀態，沒有開啟任何選單，可以購買東西，走路，進傳送門
			...
			if (nChar == KEY_F && map.GetCharacterStatus() == 3)	//買道具1
			{
			...
			if (nChar == KEY_F && map.GetCharacterStatus() == 4)	//買道具2
			{
			...
			if (nChar == KEY_F && map.GetCharacterStatus() == 5)	//買道具3
			{
